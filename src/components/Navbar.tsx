    ...
    navLinks.map((link) => {
      ...
      return (
        <Link
          key={link.name}
          to={link.path}
          className={`relative font-semibold transition-colors duration-200 ${extraClasses} ${
            isActive
              ? "text-primary-400"
              : isDarkMode
        ...
        >
          {link.name}
          <span
            className={`absolute -bottom-1 left-0 h-0.5 w-full origin-left scale-x-0 bg-gradient-to-r from-primary-500 to-amber-400 transition-transform duration-200 ${
      ...
      >
        <div className="mx-auto flex h-20 max-w-7xl items-center justify-between px-6 lg:px-10">
          <Link to={routes.Landing_page} className="flex items-center gap-3 sm:gap-4">
            <div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-gradient-to-br from-primary-600 via-emerald-500 to-amber-500 text-xl font-bold text-white shadow-lg shadow-emerald-500/20 sm:h-12 sm:w-12 sm:text-2xl">
              Y
            </div>
            <div>
              <p
                className={`text-[10px] font-semibold uppercase tracking-[0.28em] ${
                  isDarkMode ? "text-secondary-200" : "text-secondary-500"
          ...
          <div className="hidden items-center gap-10 lg:flex">
            <div className="flex items-center gap-8 text-sm">{renderLinks("group text-sm font-medium")}</div>
          ...
          <div className="flex items-center gap-4 lg:hidden">
            ...
            <button
              ...
              className={`inline-flex h-11 w-11 items-center justify-center rounded-2xl border transition focus-visible:outline-none focus-visible:ring-2 ${
                isDarkMode
